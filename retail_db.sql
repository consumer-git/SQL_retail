...
-- insert vales for customer_info, branch_details then finally sales_fact from above 
...
-- WHO are the top buyers in each state? 


select
	custname, 
	round(sum(qty)*sum(price)) as totalrevenue
from     
(
select 
	c.first_name as custname, 
  
     om.quantity_ordered as qty,
...
inner join 
 	sales_fact as sf
on
	c.customer_id=sf.customer_id 
inner join
	order_product_mapping as om
on
		om.order_id=sf.order_id 
inner join 
	product_info as pi 
on
	pi.product_id=om.product_id
where 
	om.quantity_ordered is not null and pi.sale_price is not null 	
) a 	
group by 
		custname
order by 
